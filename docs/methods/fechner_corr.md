# Fechner correlation  (`fechner_corr`)
...
```
fechner_corr(X, y)  |  UnivariateFilter('fechner_corr', ('K best', k))
```
...
```bash
itmofs-lab info fechner_corr
itmofs-lab run fechner_corr --dataset breast_cancer
```
