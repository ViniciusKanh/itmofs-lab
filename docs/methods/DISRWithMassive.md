# DISR with Massive  (`DISRWithMassive`)

**Família:** `filters.multivariate` — **tipo:** `standalone_selector`  
**Status na ITMO_FS 0.3.3:** `OK` — Executa normalmente.

Double Input Symmetric Relevance (kASSI); maximiza info mútua evitando densidade multivariada grande.

## O que entra

- **X:** sim
- **y (rótulos):** obrigatório
- **Pré-processamento:** dados discretizados; escala recomendada: `discretize`

| parâmetro | descrição |
|---|---|
| `expected_size` | nº de features esperado |

## O que sai

- sem score global ❌
- ranking ✅
- subconjunto ✅
- **Atributos após `fit`:** `selected_`, `selected_names_`
- **Transforma treino e teste:** sim

## Assinatura na ITMO_FS

```
DISRWithMassive(expected_size).fit(X,y).transform(X)
```

## Exemplo

```python
import itmofs_lab as fs
m = fs.get('DISRWithMassive')
m.fit(X_train, y_train)      # ajusta SÓ no treino
X_sel = m.transform(X_test)
print(m.selected_names_)
```

Ajuda na linha de comando:

```bash
itmofs-lab info DISRWithMassive
itmofs-lab run DISRWithMassive --dataset breast_cancer
```

**Referência ITMO_FS:** `ITMO_FS.filters.multivariate`
